...
-- `Todo.Views`' functions take a concrete `urls : Todo.Urls` (`Todo/Urls.lean`) -- `Todo.urls`
-- (the real value `urlTree` generated) is already visible unqualified here via `open Todo`, no
-- fake stand-in needed.
...
#guard filterFromPath (Filter.path urls .all) == .all
#guard filterFromPath (Filter.path urls .active) == .active
#guard filterFromPath (Filter.path urls .completed) == .completed
